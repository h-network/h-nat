...
Composed workflows can instead reference an `h_asimov_gate` instance by name
through a `gate_fn: FunctionRef` field:
...
```bash
python examples/h-asimov/standalone-gate/run_demo.py
```
...
```bash
nat validate --config_file examples/h-asimov/standalone-gate/workflow.yaml

nat run --config_file examples/h-asimov/standalone-gate/workflow.yaml \
  --input '{"command": "list the files in the current directory"}'

nat run --config_file examples/h-asimov/standalone-gate/noop.yaml \
