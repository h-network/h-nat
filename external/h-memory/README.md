...
The repository examples read Redis from the environment. `redis_url` automatically evaluates `H_NAT_REDIS_URL`, then `REDIS_URL`, falling back to `redis://localhost:6379` if omitted in the workflow YAML.

From the repository root, configure it before validating or running them:
