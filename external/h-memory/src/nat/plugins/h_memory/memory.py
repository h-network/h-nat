...
Keyspace shape follows ``<pod>:<agent>:chat:...`` per ``docs/adrs/ADR-012-redis-naming-contract.md``.

Architecture separation:
Hot-tier conversation state is written via `h-memory`, while long-term
semantic memory, hybrid retrieval, vector indexing, and background audit
sweepers are handled by `h-recall` / `h-network-semantic-memory` (per ADR-010).
What stays here is the narrow high-throughput write surface plus `delete_chat` for session cleanup.
...
Index ownership (shared substrate contract — see ``INSTALL.md`` § 7):
  ...
  - Memory module owns the **write contract**: ZADD on every write,
    ...
    to bound the ZSET against the largest TTL window, and (when ``hot_keep_count``
    is set) ZREMRANGEBYRANK per write to bound the ZSET to the most-recent N entries.
...
JSON payload ``ts`` is unix-seconds (integer seconds); sub-second ordering lives
in the key suffix and the ZSET score (both ``ts_ns``).
"""
