...
def _default_redis_url() -> str:
    """Resolve default Redis URL from environment or fall back to localhost."""
    return os.environ.get("H_NAT_REDIS_URL") or os.environ.get("REDIS_URL") or "redis://localhost:6379"
...
try:
    ...
    class FunctionInfo:  # type: ignore[no-redef]
        def __init__(self, fn: Any, converters: list[Any] | None = None, description: str = ""):
            self.fn = fn
            self.single_fn = fn
        ...
        @classmethod
        def from_fn(cls, fn: Any, converters: list[Any] | None = None, description: str = "") -> "FunctionInfo":
...
class WriteTurnInput(BaseModel):
    ...
    hot_keep_count: int | None = Field(default=None, ge=1)
...
async def h_memory_write_turn(
    ...
    Lifecycle:
      ...
      3. Emit the connect log line (structured telemetry for
         runtime diagnostics).
