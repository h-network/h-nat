...
def test_redis_url_default_without_env(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.delenv("H_NAT_REDIS_URL", raising=False)
    monkeypatch.delenv("REDIS_URL", raising=False)

    w_cfg = HMemoryWriteTurnConfig(pod="p", agent="a")
    assert w_cfg.redis_url == "redis://localhost:6379"

    d_cfg = HMemoryDeleteChatConfig(pod="p", agent="a")
    assert d_cfg.redis_url == "redis://localhost:6379"


def test_redis_url_from_h_nat_redis_url(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setenv("H_NAT_REDIS_URL", "redis://cluster-hnemo:6380")
    monkeypatch.delenv("REDIS_URL", raising=False)

    w_cfg = HMemoryWriteTurnConfig(pod="p", agent="a")
    assert w_cfg.redis_url == "redis://cluster-hnemo:6380"

    d_cfg = HMemoryDeleteChatConfig(pod="p", agent="a")
    assert d_cfg.redis_url == "redis://cluster-hnemo:6380"


def test_redis_url_from_redis_url(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.delenv("H_NAT_REDIS_URL", raising=False)
    monkeypatch.setenv("REDIS_URL", "redis://std-redis:6379")

    w_cfg = HMemoryWriteTurnConfig(pod="p", agent="a")
    assert w_cfg.redis_url == "redis://std-redis:6379"

    d_cfg = HMemoryDeleteChatConfig(pod="p", agent="a")
    assert d_cfg.redis_url == "redis://std-redis:6379"


def test_redis_url_h_nat_precedence_over_redis_url(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setenv("H_NAT_REDIS_URL", "redis://primary-hnat:6379")
    monkeypatch.setenv("REDIS_URL", "redis://fallback:6379")

    w_cfg = HMemoryWriteTurnConfig(pod="p", agent="a")
    assert w_cfg.redis_url == "redis://primary-hnat:6379"

    d_cfg = HMemoryDeleteChatConfig(pod="p", agent="a")
    assert d_cfg.redis_url == "redis://primary-hnat:6379"


def test_redis_url_explicit_override_wins_over_env(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setenv("H_NAT_REDIS_URL", "redis://env-url:6379")

    w_cfg = HMemoryWriteTurnConfig(pod="p", agent="a", redis_url="redis://explicit:6379")
    assert w_cfg.redis_url == "redis://explicit:6379"

    d_cfg = HMemoryDeleteChatConfig(pod="p", agent="a", redis_url="redis://explicit:6379")
    assert d_cfg.redis_url == "redis://explicit:6379"
