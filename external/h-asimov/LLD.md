...
- **Status**: open for lead review — this is an interpretation of the predecessor's intent (LLD.md/HLD.md's §"No-op variant" both describe `NoopFirewall` as part of the port target) rather than something the docstring spec spelled out for the NAT surface specifically.
...
- **Status**: open for lead review — same category as §5.6, an interpretation made to preserve an invariant the ported pipeline logic depends on, not literally specified for the NAT-facing output shape.
...
- **Status**: open for lead review — flagging in case the intended integration shape is actually "gate wraps execution" rather than "gate is a separate step a flow calls before execution."
