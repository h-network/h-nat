...
- **Status**: open for lead review — this is an interpretation of the predecessor's intent (LLD.md/HLD.md's §"No-op variant" both describe `NoopFirewall` as part of the port target) rather than something the docstring spec spelled out for the NAT surface specifically. (An earlier relay of the architect's feedback said this was confirmed; that was retracted as a miscommunication — still awaiting an actual decision.)
...
- **Status**: open for lead review — same category as §5.6, an interpretation made to preserve an invariant the ported pipeline logic depends on, not literally specified for the NAT-facing output shape. (Not yet confirmed either way — see the note on §5.6.)
