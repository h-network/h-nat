...
5. **Topology Flexibility**: Support both single-instance (colocated) Redis deployments and distributed split topologies where the hot tier resides on core Redis and the audit/vector tier resides on per-agent Redis Stack instances (Round 67).
...
- Eliminates RediSearch global index lock contention during heavy concurrent vector writes and searches (Round 64/67).
