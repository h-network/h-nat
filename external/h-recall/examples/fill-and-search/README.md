...
2. **Redis Stack** with `RediSearch` and `RedisJSON` modules loaded:
   ```bash
   export H_NAT_REDIS_URL=redis://172.16.10.102:6379   # or your Redis Stack URL
   ```
...
```bash
python external/h-recall/examples/fill-and-search/run_demo.py
```
...
```bash
nat validate --config_file external/h-recall/examples/fill-and-search/workflow.yaml
```
...
======================================================================
Target Redis URL: redis://172.16.10.102:6379
...
--- [Step 4/4] Executing hybrid retrieval queries via h_semantic_search ---
  ...
  [Query 4/4]: Who is the lead architect for distributed storage?
    Rank 0 [user]: "The lead architect for the distributed storage engine is Dr. Elena Rostova."
