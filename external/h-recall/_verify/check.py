"""Topology build check — exercises NAT's WorkflowBuilder
against ``build-check.yaml`` and ``build-check-split.yaml``
without invoking any function.

Pass criterion: all three function ``_type:``s build successfully (the
NAT surface — Pydantic config validation, type-hint resolution,
...
The split-topology variant declares ``hot_redis_url`` distinct from ``redis_url``
(both deliberately unreachable). A positive PASS demonstrates that BOTH the colocated and
split-topology sweep configs build correctly under the dual-client architecture.
...
async def main() -> int:
    ...
    await _build_one(here / "build-check-split.yaml", "split topology")
