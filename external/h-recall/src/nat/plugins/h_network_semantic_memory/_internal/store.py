...
NOTE on lazy connection: per round-38 announcement § 3, the store
constructor takes a redis client but does NOT issue any commands. The
first command (and the idempotent ``FT.CREATE``) only fire when a
``_type:`` actually invokes. ``redis.asyncio.Redis.from_url(url)``
returns a client without opening a TCP connection — that connection
opens on first command.
"""
import asyncio
import json
import logging
...
# shared-substrate contract (memory's INSTALL.md § 7); we never write.
HOT_SCOPE = "chat"

# This module's audit-tier scope tag (ADR-012, architect-decided per
# round-36 onboarding REPLY § 2.3).
AUDIT_SCOPE = "chat-audit"

# Round-54: the canonical TAG-value used for the ``$.pending_vectorize``
...
class AuditStore:
    ...
    **Round-67 dual-client.** The store now holds two redis clients:
    ...
    dual-client shapes:
      ...
      - ``AuditStore(client=c, ...)`` — colocated topology; the same
        client services both tiers. Pre-round-67 default; still the
        documented default when ``hot_redis_url`` is omitted from the
        function config.
      - ``AuditStore(audit_client=ac, hot_client=hc, ...)`` — split
        topology; explicit per-tier clients (operator's per-agent
        vector Redis architectural pattern, per round 64+).
    ...
    def client(self) -> aioredis.Redis:
        ...
        that pre-date the round-67 dual-client split see the audit
        client by default — round-67's classification audit
        (cf. ``store.py`` class docstring) showed every prior
        ``store.client`` reference was on the audit side. Internal
        callers should use the explicit property; this alias exists
        for any external code that pinned on the old name."""
        return self._audit_client
    ...
    async def ensure_index(self) -> None:
        ...
        Schema (round-54 update):

          - TAG ``$.chat_id``           — tenancy filter.
          - TAG ``$.role``              — role filter.
          - TAG ``$.pending_vectorize`` — round-54: enables FT.SEARCH-based
        ...
        **Upgrade note** (round 38 → round 54): existing audit indexes
        created against round-38's schema lack the
        ``$.pending_vectorize`` TAG field. They will not match
        ``@pending_vectorize:{1}`` queries; vectorize will discover
        zero pending docs forever. Drop and re-create the index, or
        ``FT.ALTER`` if your RediSearch version supports adding a
        JSON-path field to a JSON-typed index. See INSTALL.md
        "Upgrade notes" for the full deployment story (architect-decided
        per ADR-009).
        """
    ...
    async def write_audit(
        ...
        **Round-54 encoding choice.** When ``pending_vectorize=True``,
    ...
    async def read_hot(self, key: str) -> Optional[dict[str, Any]]:
    ...
    async def _search_to_docs(
        ...
        re-introduce the round-5 RedisJSON float round-trip risk).
