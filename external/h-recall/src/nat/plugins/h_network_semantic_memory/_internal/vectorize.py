...
Discovery shape — FT.SEARCH on the indexed flag:
``store.ensure_index()`` declares a TAG field on ``$.pending_vectorize``;
FT.SEARCH ``@pending_vectorize:{1}`` returns only docs that still carry the flag,
so the discovery cost is O(actual_pending_count), regardless of how big the audit
corpus has grown.

The encoding ``"1"`` (JSON string, not Python ``True``) is used —
see :func:`store.write_audit` docstring + the
...
from .store import PENDING_VECTORIZE_TAG_VALUE, AuditStore
