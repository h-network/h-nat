...
LLD-locked behavior (see ``store.py`` § "LLD invariants"):
  ...
  - The audit doc carries a sentinel ``[0.0]*384`` embedding plus
    ``$.pending_vectorize: "1"`` — vectorize fills the real vector
    asynchronously; the sentinel keeps the doc visible to FT.SEARCH
    text queries immediately.

Discovery shape — chat-index ZSET driven:
Discovery uses ZRANGEBYSCORE on memory module's
``<pod>:<agent>:chat-index:<chat_id>`` per-chat ZSETs (cross-module
read per memory ``INSTALL.md`` § 7.2). The cost is
O(sum_of_per_chat_hot_counts), independent of audit corpus size.

Consumer-tracks-everything posture: the consumer
...
Per-call bounds:
  ...
  - ``migration_threshold_sec`` — turns whose ``ts_ns`` is older than
    ``now - migration_threshold_sec`` are migrated using the
    chat-index ZSET score directly.
...
Returns:

    {
      ...
      "skipped_fresh":    int,  # always 0 (kept for return shape parity)
...
``INSTALL.md`` documents. The race against another reader
...
async def migrate(
    ...
    *,
    ...
    # Age cutoff: turn_keys with score (ts_ns) ≤ cutoff_ns are old
        ...
        for turn_key in turn_keys:
            ...
            if await store.audit_exists(long_key):
                ...
                try:
                    ...
                    logger.debug(
                        "migrate: lazy zrem cleanup for already-migrated %s failed",
                        turn_key,
                        exc_info=True,
                    )
                skipped_existing += 1
            ...
            try:
                ...
                logger.debug(
                    "migrate: lazy zrem cleanup for %s failed",
                    turn_key,
                    exc_info=True,
                )
