...
Design invariants:
  ...
  - **NO ``from __future__ import annotations``** at module top — NAT's
    ``FunctionInfo.from_fn`` introspects via ``typing.get_type_hints``
    against an internally-generated wrapper.
  - **``str → PydanticModel`` converters** registered alongside each
    function so ``nat run --input '<JSON>'`` (CLI) and any structured-input
    front-ends bridge ``str`` to the input dataclass.
...
class HSemanticSearchConfig(FunctionBaseConfig, name="h_semantic_search"):
    ...
    embed_model: str = Field(
        ...
        description=(
            "fastembed model name. Schema-locked at 384d; "
...
class HSemanticSweepConfig(FunctionBaseConfig, name="h_semantic_sweep"):
    ...
    redis_url: str = Field(
        ...
        description=(
            ...
            "Redis Stack instance under a split topology, or at a colocated Redis "
    ...
    hot_redis_url: str | None = Field(
        ...
        description=(
            ...
            "tiers — colocated topology. "
            "Set explicitly to enable the split topology."
        ),
    )
    ...
    migration_threshold_sec: int = Field(
        default=18000, ge=1,  # 5h default
        description=(
            "Age threshold for migration: hot turns whose "
...
class SemanticSweepInput(BaseModel):
    """Per-call input for ``h_semantic_sweep``.

    Sweep operates on the chats the consumer tracks. The consumer
    (workflow / harness / overlay) is responsible for tracking which
    chats have hot turns needing migration.
    ...
    - ``chat_ids=["c1","c2"]`` → sweep each chat via its
      ...
      cross-module read contract per ``INSTALL.md``).
    """
    model_config = ConfigDict(extra="forbid")
...
async def h_semantic_sweep(
    ...
    Dual-Redis support: ``hot_redis_url`` (optional) routes chat-index
    ZRANGEBYSCORE / hot-key reads / ZREM-on-success at memory module's
    substrate; ``redis_url`` targets the audit-tier (vector) Redis. If
