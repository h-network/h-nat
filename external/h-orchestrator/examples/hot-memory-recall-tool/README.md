...
- The five h-nat packages plus NAT's LangChain agent plugin.
...
```bash
pip install -e external/h-openshell
...
for config in workflow.yaml sweep.yaml vectorize.yaml; do
  nat validate --config_file \
    "external/h-orchestrator/examples/hot-memory-recall-tool/$config"
done
```
...
```bash
python external/h-orchestrator/examples/hot-memory-recall-tool/run_demo.py
```
...
```bash
nat run --config_file external/h-orchestrator/examples/hot-memory-recall-tool/workflow.yaml \
