...
Repository-level integration assets live under
`examples/h-orchestrator/` (`hot-memory-recall-tool`, `gated-junos-mcp`, and
`plain-chat-memory`). They are deliberately outside the module tree and are
not included in the wheel payload.

The distribution is `h-orchestrator`, version `0.1.0`, supporting Python
3.11-3.13. Runtime dependencies include `h-asimov`, `h-memory`, Redis 7.1,
`nvidia-nat-core>=1.8,<2`, `nvidia-nat-mcp>=1.8,<2`, and Pydantic 2.
`h-openshell` is available through the `openshell` extra and is not needed for
chat-cycle or gated-MCP use. The namespace package is
...
The builder resolves the parser during workflow construction but does not
import the optional OpenShell package, read its configuration, or open a
client. The first invocation imports `h-openshell` and constructs
`OpenShellClient.from_default_home`; if the extra is absent, it raises an
actionable error naming `pip install 'h-orchestrator[openshell]'`. It then
optionally prepends static context, builds the script, and calls the parser.
The reserved parser `step_manager` argument is currently `None`; intermediate-
step internals are not part of the stable external plugin API.
...
1. reads indexed h-memory payloads and orders live, valid JSON objects oldest
   first;
2. builds the documented history/current-message prompt;
...
Each sandbox-backed NAT builder closure owns at most one OpenShell client.
Optional-package import and client creation are lazy and guarded by a
per-builder `asyncio.Lock` with a second check inside the critical section, so
concurrent first invocations share one client. Teardown is deterministic.
...
The unit suite runs without h-openshell or a live gateway. It imports the full
plugin registration, verifies non-sandbox functions remain discoverable, and
uses fake clients for execution behavior.

The lightweight NAT registration doubles mirror NAT 1.8's public lifecycle:
registered factories are async context managers and yielded `FunctionInfo`
objects expose their callable as `single_fn`. This keeps direct builder tests
consistent whether NAT plugin discovery imported a module before the fixture
or the fixture imported it itself.
...
- import without h-openshell and actionable missing-extra failure;
...
- Unary invocation and agent streaming return failures as data, as documented
  by their public contracts.
- Streaming output is raw stdout and does not use the unary parser registry.
- Full OpenShell transport behavior requires an OpenShell gateway; unit tests
  use protocol-compatible fake clients.
- MCP output is text because NVIDIA NAT MCP 1.8 reduces tool results to strings.
...
Direct SSH execution is not registered by this package. External tool access
for OpenAI-style agents uses `h_gated_mcp_tool`, demonstrated by
