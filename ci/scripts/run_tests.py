...
nat validate runs against every examples/**/*.yaml with a top-level `workflow:`
key, using dummy values for the ${VAR} placeholders those configs interpolate
(H_NAT_LLM_MODEL etc.) -- validate only checks schema/config shape and makes no
network calls, so no real Redis/vLLM/Junos endpoint is needed.
"""
...
from discover_modules import REPO
